...
export class LocalBackend {
  ...
  async resolveRepo(repoParam?: string): Promise<RepoHandle> {
    ...
    if (repoParam) {
      // Echo the selector only while the reply stays bounded on both transports, and
      // echo it JSON-escaped so quotes and line breaks cannot deform the text reply.
      const escaped = JSON.stringify(repoParam);
      const shown =
        Buffer.byteLength(escaped) <= 122
          ? escaped
          : `selector of ${Buffer.byteLength(repoParam)} bytes`;
  ...
  private async detectChanges(
    ...
    for (const header of droppedHeaders) {
      ...
      if (quoted) {
        ...
        named = Buffer.from(bytes).toString('utf-8');
      } else {
        named = header.split(' ')[0];
      }
      gaps.push({
        path: named.replace(/^a\//, ''),
        reason: 'git quoted this path, so its diff header could not be mapped to a file',
      });
    }
    ...
    // set is readable: zero means no caller is known at all, a non-zero count means callers
    // are known. An incomplete walk measured nothing, and zero is already a measurement, so
    // both fields are null there rather than a number nobody checked.
    for (const sym of changedSymbols) {
      sym.incoming_edges = traversalComplete ? (edgesIntoSeed.get(String(sym.id)) ?? 0) : null;
      if (sym.incoming_edges === 0) uncoveredSymbols += 1;
    }
    if (!traversalComplete) uncoveredSymbols = null;
